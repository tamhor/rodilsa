...
class Statement_model extends CI_Model
{
    ...
    public function fetch_transasctions($date1, $date2)
    {
        ...
        $this->db->select("mp_generalentry.id as transaction_id,mp_generalentry.id_head,mp_generalentry.date,mp_generalentry.naration");
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($transaction_records  != NULL) {
                ...
                foreach ($transaction_records as $transaction_record) {
                    ...
                    $this->db->select("mp_sub_entry.*,mp_head.name,mp_head.id_head");
                        ...
                        if ($sub_query != NULL) {
                            foreach ($sub_query as $single_trans) {
                                ...
                                if ($single_trans->type == 0) {
                                    $total_debit += $single_trans->amount;
                                    $form_content .= '<tr><td>' . $transaction_record->date . '</td><td>' . $single_trans->id_head . '</td><td><a href="#">' . $single_trans->name . '</a></td><td>
                                <a href="">' . format_rp($single_trans->amount) . '</a>
                            </td>
                            <td>
                                <a href="#"></a>
                            </td>          
                                    ...
                                    $total_kredit += $single_trans->amount;

                                    $form_content .= '<tr><td>' . $transaction_record->date . '</td><td>' . $single_trans->id_head . '</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="general-journal-credit" href="#">' . $single_trans->name . '</a>
                            </td>
                            <td>
                                <a href="#"></a>
                            </td>
                    ...
                    $form_content .= '<tr class="narration" ><td class="border-bottom-journal" colspan="5"><small> <i> - ' . $transaction_record->naration . '</i>
                        </small></td></tr>';
                }
                $form_content .= '<tr><th colspan="3">Total</th><td>' . format_rp($total_debit) . '</td><td>' . format_rp($total_kredit) . '</td></tr>';
            }
        }
        return $form_content;
    }
                ...
                foreach ($heads_record as $single_head) {
                    if ($this->get_ledger_transactions($single_head->id, $date1, $date2) != NULL) {
                        ...
                        <div class=" ledger_row_head" style=" text-transform:uppercase;">
                                <b>' . $single_head->id_head . '</b>
                                <b>' . $single_head->name . '</b>
                        </div>
         
                        <thead class="ledger-table-head">
                             <th class="col-md-2">TANGGAL</th>
                             <th class="col-md-4">NAMA TRANSAKSI</th>
                        ...
                        <td>' . $single_ledger->date . '</td><td><a href="#">' . $single_ledger->naration . '</a></td><td>
                            <a href="#">' . format_rp($debitamount) . '</a>
                        </td>
                        <td>
                            <a href="#">' . format_rp($creditamount) . '</a>
                        </td>
                        <td>' . ($total_ledger < 0 ? '(' . -$total_ledger . ')' : format_rp($total_ledger)) . '</td>            
                    </tr>';
                        }
                    }
    ...
    {
        $count_total_amt = 0;
        $this->db->select("mp_generalentry.id as transaction_id,mp_generalentry.date,mp_sub_entry.*");
        $this->db->from('mp_sub_entry');
        $this->db->join('mp_generalentry', 'mp_generalentry.id = mp_sub_entry.parent_id');
            ...
            if ($ledger_data != NULL) {
                foreach ($ledger_data as $single_head) {
                    ...
                    if ($amount != NULL) {
                        ...
                        $from_creator .= '<td style=text-align:left;><h4>' . $single_head->id_head . '</h4></td><td><h4>' . $single_head->name . '</h4></td><td><h4>' . format_rp($debitamt) . '</h4></td><td><h4>' . format_rp($creditamt) . '</h4></td></tr>';
                    }
                }

                $from_creator .= '<tr class="balancesheet-row"><td></td><td></td><td><h4>' . format_rp($total_debit) . '</h4></td><td><h4>' . format_rp($total_credit) . '</h4></td></tr>';
            }
        }
    ...
    public function income_statement($date1, $date2)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($record_data != NULL) {
                ...
                $from_creator .= '<tr><td align="left" colspan="2"><span class="income-style-sub"><b> Kode Akun & Nama Akun </b></span></td></tr>';
                ...
                foreach ($record_data as $single_head) {
                    ...
                    if ($amount != 0) {
                        ...
                        $from_creator .= '<tr><td align="left"><h4>' . $single_head->id_head . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $single_head->name . '</h4></td><td class="pull-right"><h4>' . format_rp($amount) . '</h4></td></tr>';
                    }
                }

                $from_creator .= '<tr><td> Total Revenue / Pendapatan</td><td class="pull-right"><h4><b>' . format_rp($total_revenue) . '</b></h4></td></tr>';
            }
        }
        ...
        if ($query->num_rows() > 0) {
            $record_data =  $query->result();
            if ($record_data != NULL) {
                $from_creator .= '<tr><td align="left" colspan="10"><h4 class="income-style"><b>- Expense / Pengeluaran</b></h4></tr>';
                $from_creator .= '<tr><td align="left" colspan="2"><span class="income-style-sub"><b> Kode Akun & Nama Akun </b></span></td></tr>';
                ...
                $from_creator .= '<tr><td> Total Expense / Pengeluaran </td><td class="pull-right">' . format_rp($total_expense) . '</td></tr>';

                $from_creator .= '<tr class=" total-income"><td> Total Untung / Rugi </td><td class="pull-right">' . format_rp($total_revenue - $total_expense) . '</td></tr>';
            }
        }
