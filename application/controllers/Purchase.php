...
class Purchase extends CI_Controller
{
	...
	public function add_purchase()
	{
		...
		else
		{
			...
			{
				$array_msg = array(
					'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"></i> Transaksi Pembelian Berhasil Ditambahkan',
					'alert' => 'info'
				);
				$this->session->set_flashdata('status', $array_msg);
			}
			...
			{
				$array_msg = array(
					'msg' => '<i style="color:#c00" class="fa fa-exclamation-triangle" aria-hidden="true"></i> Transaksi Pembelian Tidak Berhasil Ditambahkan',
					'alert' => 'danger'
				);
				$this->session->set_flashdata('status', $array_msg);
			}
