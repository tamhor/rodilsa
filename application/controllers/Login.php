...
class Login extends CI_Controller
{
	...
	public function authentication()
	{
		...
		if (!empty($user_name) && !empty($user_password))
		{
			...
			$result = $this->Crud_model->authenticate_user($user_name, $user_password);
			if ($result == NULL)
			{
				$array_msg = array(
					'msg' => '<i style="color:#fff" class="fa fa-exclamation-triangle" aria-hidden="true"></i> Username atau Kata Sandi yang anda masukkan salah',
					'alert' => 'danger'
				);
		...
		{
			$array_msg = array(
				'msg' => '<i style="color:#fff" class="fa fa-exclamation-triangle" aria-hidden="true"></i> Masukan Username dan Kata Sandi',
				'alert' => 'danger'
			);
			$this->session->set_flashdata('status', $array_msg);
			redirect('login');
	...
	public function authentication_user()
	{
		...
		if (!empty($user_name) && !empty($user_password))
		{
			...
			// DEFINES LOAD CRUDS_MODEL FORM MODELS FOLDERS
			$this->load->model('Crud_model');
			$result = $this->Crud_model->authenticate_panel_user($user_name, $user_password);
			if ($result === FALSE)
			{
				$array_msg = array(
	...
	function recover_password_user()
	{
		...
		if (!empty($user_password) && !empty($user_cpassword))
		{
			...
			// DEFINES LOAD CRUDS_MODEL FORM MODELS FOLDERS
			$this->load->model('Crud_model');
			$result_data = $this->Crud_model->fetch_forget_password_user($user_name,$user_code);
	...
	function recover_password()
	{
		...
		if (!empty($user_password) && !empty($user_cpassword))
		{
			...
			// DEFINES LOAD CRUDS_MODEL FORM MODELS FOLDERS
			$this->load->model('Crud_model');
			$result_data = $this->Crud_model->fetch_forget_password($user_name,$user_code);
