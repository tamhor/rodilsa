...
class Invoice extends CI_Controller
{
	...
	public function edit_invoice()
	{
		...
		$edit_sph = html_escape($this->input->post('edit_sph'));
		$edit_cyln = html_escape($this->input->post('edit_cyln'));
        $edit_xis = html_escape($this->input->post('edit_xis'));
		...
		if ($result != NULL)
		{
			...
			$array_msg = array(
				'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"></i> Data Transaksi Penjualan Berhasil Diubah',
				'alert' => 'info'
				);
			$this->session->set_flashdata('status', $array_msg);
		}
		...
		{
			$array_msg = array(
				'msg' => '<i style="color:#c00" class="fa fa-exclamation-triangle" aria-hidden="true"/> Data Transaksi Penjualan Gagal Diubah',
				'alert' => 'danger'
			);
			$this->session->set_flashdata('status', $array_msg);
		}
	...
	function add_auto_invoice()
	{
		...
		if($result != NULL)
		{
			...
			if ($data != NULL)
			{
				...
				$array_msg = array(
					'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"></i> Transaksi berhasil ditambahkan',
					'alert' => 'info'
					);
