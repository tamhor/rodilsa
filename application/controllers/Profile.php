	...
	public function change_password()
	{
		...
		if ($new_login_customer == $confirm_password) {
			...
			if (sha1($old_password) == $saved_password) {
				...
				if ($result == 1) {
					$array_msg = array(
						'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"/> pengubahan data berhasil',
						'alert' => 'info'
					);
					$this->session->set_flashdata('status', $array_msg);
				} else {
					$array_msg = array(
						'msg' => '<i style="color:#fff" class="fa fa-exclamation-triangle" aria-hidden="true"/> pengubahan data gagal',
						'alert' => 'danger'
					);
					$this->session->set_flashdata('status', $array_msg);
				}
			...
			$array_msg = array(
				'msg' => '<i style="color:#fff" class="fa fa-exclamation-triangle" aria-hidden="true"/> Password tidak sama silahkan periksa kembali',
				'alert' => 'danger'
			);
			$this->session->set_flashdata('status', $array_msg);
		}
