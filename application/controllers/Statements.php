...
class Statements extends CI_Controller
{
	...
	function add_new_balance()
	{
		...
		if ($result != NULL) {
			$array_msg = array(
				'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"></i> Data Entri Jurnal Berhasil Ditambahkan',
				'alert' => 'info'
			);
			$this->session->set_flashdata('status', $array_msg);
		} else {
			$array_msg = array(
				'msg' => '<i style="color:#c00" class="fa fa-exclamation-triangle" aria-hidden="true"></i> Data Entri Jurnal Tidak Berhasil Ditambahkan',
				'alert' => 'danger'
			);
			$this->session->set_flashdata('status', $array_msg);
		}
